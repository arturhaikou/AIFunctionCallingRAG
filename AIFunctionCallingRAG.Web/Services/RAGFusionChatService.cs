...
{
    public class RAGFusionChatService
    {
        ...
        public RAGFusionChatService(string key, string model, IEshopApi api, string assistantMessage)
        {
            _api = api;
        ...
        private class KernelPlugin(RAGFusionChatService chatService)
        {
            [KernelFunction("search_products"), Description("Search products by specific criteria like product name, color")]
            public async Task<string> Search(
                [Description("The name of the product from the user question.")] string name,
                [Description("The color of the product from the user question")] string color)
            {
                var response = await chatService._api.HybridSearchAsync(name, color);
                return response;
            }
